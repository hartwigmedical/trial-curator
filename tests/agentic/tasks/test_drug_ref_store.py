...
def test_save_then_load_round_trips_all_tables(tmp_path):
    ...
    s.add_occurrence("NCT01", "ctgov", "Keytruda")            # provenance: two trials use one input name
    s.add_occurrence("NCT02", "ctgov", "Keytruda")
    s.add_occurrence("ACTRN99", "anzctr", "pembrolizumab")
    ...
    # provenance round-trips (3 distinct trial links)
    assert {(o.trialId, o.registry) for o in loaded.occurrences.values()} == {
        ("NCT01", "ctgov"), ("NCT02", "ctgov"), ("ACTRN99", "anzctr")}
...
def test_legacy_drug_alias_still_loads(tmp_path):
    """Backward compat: a pre-split version dir with a legacy drug_alias.tsv still loads into mappings."""
    vdir = tmp_path / "version_10072026"
    vdir.mkdir()
