...
def test_anzctr_cancer_type_discovery_uses_shared_resource_defaults(tmp_path: Path):
    eligibility_dir = tmp_path / "data/eligibility_path/exports/intermediates/anzctr"
    write(
        tmp_path / "data/trial_inputs/anzctr/extracted_trials/anzctr_field_extractions.csv",
        "ACTRN,HEALTH CONDITION\n12605000003673,Breast cancer\n",
    )
    write(
        tmp_path / "data/trial_inputs/anzctr/eligibility_curations/ACTRN12605000003673.py",
        "rules = []\n",
    )

    shared_resources = tmp_path / "data/eligibility_path/resources"
    write(shared_resources / "oncotree/oncotree.csv", "code,name,parent\n")
    write(
    ...
    assert inputs.input_csv == tmp_path / "data/trial_inputs/anzctr/extracted_trials/anzctr_field_extractions.csv"
    ...
    assert inputs.oncotree_csv == shared_resources / "oncotree/oncotree.csv"
    assert inputs.output_dir == eligibility_dir / "cancer_type"
