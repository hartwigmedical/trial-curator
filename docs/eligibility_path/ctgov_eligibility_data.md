...
The raw JSON is the source registry input. The extracted trial CSV is generated
from that JSON. The `NCT*.py` files are pydantic curator outputs and are treated
as source inputs for the downstream eligibility processing stages.
