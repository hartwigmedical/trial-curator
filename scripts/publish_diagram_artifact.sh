#!/usr/bin/env bash
#
# Publish docs/diagrams/v2_eligibility_workflow_diagram.html to its Claude Artifact, OVERWRITING the existing one.
#
...
# ask Claude in a normal session: "re-publish docs/diagrams/v2_eligibility_workflow_diagram.html, overwriting <URL>".

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
DIAGRAM="${REPO_ROOT}/docs/diagrams/v2_eligibility_workflow_diagram.html"
