...
BASE_CMD="docker run --mount "type=bind,src=${2},dst=/actin_data" --mount "type=bind,src=${gc_home},dst=/root/.config/gcloud,ro" $1"
echo "Running using Vertex"
$BASE_CMD

if [[ $# -eq 3 ]]; then
  echo "Running using OpenAI"
  $BASE_CMD -e LLM_PROVIDER=OpenAI -e OPENAI_API_KEY="$3" 
else
