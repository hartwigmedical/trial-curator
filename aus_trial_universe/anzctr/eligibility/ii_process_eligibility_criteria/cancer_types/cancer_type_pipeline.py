...
DEFAULT_ELIGIBILITY_DATA_DIR = Path("data/anzctr/eligibility")
...
DEFAULT_INPUT_CSV = Path("trials/anzctr_field_extractions_w_drugs.csv")

HEALTH_CONDITION_MAPPING_STEM = "01_health_condition_mapping"
PRIMARY_VS_HEALTH_CONDITION_STEM = "02_primary_vs_health_condition"
PRIMARY_VS_HEALTH_CONDITION_CHECKS_STEM = "02a_primary_vs_health_condition_checks"
...
MANUAL_REVIEW_COLUMNS: Sequence[str] = (
    ...
    "conditions_oncotree_curation_cleaned_simplified",
    RELATION_COL,
    "inclusive_rule",
    "cohorts",
    "ancestor_chain",
...
class AnzctrCancerTypePipelineOutputs:
    ...
    primary_vs_health_condition_checks_file: Path
...
def _has_actrn_py_files(path: Path) -> bool:
    if path.is_file():
        return path.name.upper().startswith("ACTRN") and path.suffix == ".py"
    if not path.is_dir():
        return False
    return any(path.glob("ACTRN*.py"))


def _iter_actrn_py_files(curated_dir: Path) -> Iterable[Path]:
    if curated_dir.is_file():
        yield curated_dir
        return
    yield from sorted(path for path in curated_dir.glob("ACTRN*.py") if path.is_file())
...
def relation_contains_check(value: object) -> bool:
    text = _normalise_manual_value(value)
    if not text:
        return False
    return any(part.strip().casefold() == "check" for part in text.split("|"))


def build_unresolved_check_review_table(df: pd.DataFrame) -> pd.DataFrame:
    if RELATION_COL not in df.columns:
        raise ValueError(f"Input dataframe missing required column: {RELATION_COL}")
    if MANUAL_OVERWRITE_COL not in df.columns:
        raise ValueError(f"Input dataframe missing required column: {MANUAL_OVERWRITE_COL}")

    mask = [
        relation_contains_check(relation) and not _normalise_manual_value(manual_overwrite)
        for relation, manual_overwrite in zip(df[RELATION_COL], df[MANUAL_OVERWRITE_COL])
    ]
    ...
    return out.loc[:, list(MANUAL_REVIEW_COLUMNS)].reset_index(drop=True)
...
def build_primary_vs_health_condition_table(
    *,
    ...
    for py_path in _iter_actrn_py_files(curated_dir):
        n_files += 1
...
def discover_pipeline_inputs(
    *,
    ...
    resolved_input_csv = (
        ...
        else resolved_eligibility_data_dir / DEFAULT_INPUT_CSV
    )
    if input_csv is None and not resolved_input_csv.exists():
        resolved_input_csv = _find_best_file(
            resolved_eligibility_data_dir / DEFAULT_TRIALS_SUBDIR,
...
def validate_pipeline_inputs(inputs: AnzctrCancerTypePipelineInputs) -> None:
    ...
    if not _has_actrn_py_files(inputs.curated_dir):
        raise FileNotFoundError(
...
def run_cancer_type_pipeline(
    ...
    primary_vs_health_condition_checks_file = _output_path(
        inputs.output_dir,
        PRIMARY_VS_HEALTH_CONDITION_CHECKS_STEM,
        inputs.output_format,
    )
    ...
    unresolved_check_df = build_unresolved_check_review_table(primary_vs_health_condition_df)
    if not unresolved_check_df.empty:
        _write_tabular_file(unresolved_check_df, primary_vs_health_condition_checks_file)
        raise ValueError(
            "ANZCTR cancer-type processing found "
            f"{len(unresolved_check_df)} unresolved CHECK row(s). "
            f"Review {primary_vs_health_condition_checks_file}, fill {MANUAL_OVERWRITE_COL!r} "
            "where needed, then rerun with "
            f"--manual_overwrite_file {primary_vs_health_condition_checks_file}."
        )
    ...
    logger.info("Wrote row-level cancer type table: %s rows=%d", row_level_file, len(row_level_df))
    ...
    return AnzctrCancerTypePipelineOutputs(
        ...
        primary_vs_health_condition_checks_file=primary_vs_health_condition_checks_file,
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        help="ANZCTR eligibility data directory. Defaults to data/anzctr/eligibility.",
    )
    parser.add_argument(
        "--input_csv",
        type=Path,
        ...
        help=(
            ...
            "eligibility_data_dir/trials/anzctr_field_extractions_w_drugs.csv."
        ),
    )
    parser.add_argument(
        "--curated_dir",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Shared eligibility resources directory. Defaults to data/eligibility/resources.",
    )
    parser.add_argument(
        "--cancer_type_resource_dir",
        type=Path,
    ...
    logger.info(
        "primary_vs_health_condition_checks_file: %s",
        outputs.primary_vs_health_condition_checks_file,
    )
