...
def _try_inject_missing_name_from_exception(module_globs: dict[str, Any], exc: BaseException) -> bool:
    ...
    if name.startswith("_"):
        module_globs[name] = _make_shim(name)
        logger.debug("Injected shim for missing internal symbol %r into exec globals.", name)
        return True
    ...
    if obj is not None:
        ...
        logger.debug("Injected missing symbol %r into exec globals (from typing/typing_extensions).", name)
        return True

    # Fallback: shim it
    module_globs[name] = _make_shim(name)
    logger.debug("Injected shim for missing symbol %r into exec globals (fallback).", name)
    return True
...
def _maybe_fix_typing_specialform_typeerror(module_globs: dict[str, Any], exc: BaseException) -> bool:
    ...
    if changed:
        logger.debug("Replaced typing internals with shims after _SpecialForm TypeError; retrying exec once.")
    return changed
...
def load_curated_rules(py_filepath: Path) -> list[Any] | None:
    ...
    try:
        ...
        logger.debug(
            "SyntaxError in %s:%s; attempting automatic repair: %s",
            py_filepath,
            e.lineno,
            e.msg,
        )
        ...
        for lbl, fixer in fixers:
            ...
            if new_fixed != fixed:
                logger.debug("Auto-fix (%s) applied to %s", lbl, py_filepath)
                fixed = new_fixed
                changed = True
        ...
        if rules is not None:
            logger.debug("Auto-fix succeeded for %s; using repaired source.", py_filepath)
        return rules
