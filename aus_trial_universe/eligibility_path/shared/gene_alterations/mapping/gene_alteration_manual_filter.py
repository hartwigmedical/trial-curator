...
def build_manual_filter_report(
    *,
    ...
    py_files = list(iter_curated_py_files(input_dir, trial_id_prefix=trial_id_prefix))
...
def _cohort_base_with_trial_id_column(cohort_base_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "trial_id" in cohort_base_df.columns:
        return cohort_base_df.rename(columns={"trial_id": "TrialId"}).copy()

    if "nct_id" not in cohort_base_df.columns:
        raise ValueError(
            "Cohort base table must contain one of 'trial_id', 'nct_id', or 'TrialId'. "
