...
def _cohort_keys_in_base_order(
    ...
    for _, row in cohort_base_df.iterrows():
        trial_id = normalize_anzctr_trial_id(row.get(TRIAL_ID_COL, ""))
...
def collapse_effective_cancer_type_rows_to_cohort_level(
    ...
    *,
    ...
    """
    Collapse already-expanded effective cancer-type rows to trial_id + cohort.
...
def collapse_to_cohort_level_cancer_type(
    *,
    ...
    """
    Collapse row-level cancer-type output to trial_id + cohort.
...
def discover_cohort_level_cancer_type_inputs(
    *,
    ...
    resolved_oncotree_csv = (
        ...
        else _resolve_path(DEFAULT_SHARED_RESOURCES_DIR, repo_root) / DEFAULT_ONCOTREE_CSV
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "eligibility_data_dir/cancer_type."
        ),
    )
    parser.add_argument(
        "--output_format",
