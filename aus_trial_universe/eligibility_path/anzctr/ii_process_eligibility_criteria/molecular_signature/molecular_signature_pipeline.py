...
def build_mapped_criteria_table(
    *,
    ...
    py_files = list(iter_curated_py_files(curated_dir, trial_id_prefix="ACTRN"))
...
def collapse_to_trial_level(mapped_df: pd.DataFrame) -> pd.DataFrame:
    ...
    required = ["trial_id", "polarity", "molecular_signature_curation"]
    ...
    grouped = mapped_df.groupby("trial_id", sort=False, dropna=False)
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the ANZCTR molecular-signature eligibility pipeline: copy the "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Directory containing original curated ACTRN*.py files, or a single ACTRN*.py file. "
