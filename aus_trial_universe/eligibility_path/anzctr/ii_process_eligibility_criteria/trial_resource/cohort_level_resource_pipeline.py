...
from aus_trial_universe.eligibility_path.anzctr.ii_process_eligibility_criteria.trial_resource.trial_level_resource_pipeline import (
    ...
    OUTPUT_TRIAL_ID_COLUMN,
    _count_nonblank,
    _find_trial_id_column,
    _load_base_trials,
...
def _normalize_cohort_base_table(cohort_base_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in cohort_base_df.iterrows():
        trial_id = _normalize_anzctr_trial_id(row.get("trial_id", ""))
    ...
    return pd.DataFrame(
        rows,
        columns=["_trial_id_key", "_cohort_key", COHORT_COLUMN],
    )
...
def _load_or_build_cohort_base(
    ...
    if cohort_base_file is not None:
        ...
        trial_id_col = _find_trial_id_column(
            df,
        ...
        normalized = df.rename(columns={trial_id_col: "trial_id", cohort_col: COHORT_COLUMN})
        ...
        LOGGER.info(
            ...
            cohort_base_file,
            len(out),
            out["_trial_id_key"].nunique() if not out.empty else 0,
        )
        return out
    ...
    built = build_cohort_base_from_curated_rules(
        ...
        normalize_trial_id=_normalize_anzctr_trial_id,
    )
...
def _base_trial_keys_in_order(base_df: pd.DataFrame) -> List[str]:
    ...
    for value in base_df.get("_trial_id_key", pd.Series(dtype=str)).tolist():
        trial_id = _normalize_anzctr_trial_id(value)
...
def _complete_cohort_base_for_base_trials(
    ...
    The final cohort resource is defined on the ANZCTR field extraction base.
    ...
    for _, row in cohort_base_df.iterrows():
        trial_id = _normalize_anzctr_trial_id(row.get("_trial_id_key", ""))
        cohort = normalize_cohort_label(row.get(COHORT_COLUMN, ""))

        if not trial_id or not cohort:
            continue

        if trial_id not in base_trial_key_set:
            extra_trial_keys.add(trial_id)
            continue

        cohort_lookup.setdefault(trial_id, [])
        if cohort not in cohort_lookup[trial_id]:
            cohort_lookup[trial_id].append(cohort)
    ...
    for trial_id in base_trial_keys:
        cohorts = cohort_lookup.get(trial_id, [])
...
def _expand_base_trials_to_cohorts(
    ...
    _require_columns(base_df, ["_trial_id_key"], label="Base trials table")
    _require_columns(
        cohort_base_df,
        ["_trial_id_key", "_cohort_key", COHORT_COLUMN],
        label="Cohort base table",
    )
    ...
    LOGGER.info(
        ...
        len(base_df),
        len(expanded),
        expanded["_trial_id_key"].replace("", pd.NA).dropna().nunique(),
    )
...
def _load_cohort_component_table(
    ...
    out = out[
        out["_trial_id_key"].ne("") & out["_cohort_key"].ne("")
        ].copy()

    duplicated = out.loc[
        out.duplicated(["_trial_id_key", "_cohort_key"], keep=False),
        ["_trial_id_key", "_cohort_key"],
    ]
    if not duplicated.empty:
        duplicate_key_rows = duplicated.drop_duplicates().to_numpy().tolist()
        examples = [
            f"{trial_id}/{cohort}"
            for trial_id, cohort in duplicate_key_rows
        ]
        raise ValueError(
            f"{label} has duplicate trial_id + cohort rows for "
            f"{len(examples)} key(s): {', '.join(examples[:20])}"
        )

    out = out.loc[:, ["_trial_id_key", "_cohort_key", *required_value_columns]]
    ...
    LOGGER.info(
        ...
        out[["_trial_id_key", "_cohort_key"]].drop_duplicates().shape[0],
...
def _component_keys_not_in_base(
    ...
    base_keys = set(
        map(
            tuple,
            base_df.loc[:, ["_trial_id_key", "_cohort_key"]].drop_duplicates().to_numpy(),
        )
    )
    component_keys = set(
        map(
            tuple,
            component_df.loc[:, ["_trial_id_key", "_cohort_key"]]
            .drop_duplicates()
            .to_numpy(),
        )
    )
...
def _merge_cohort_component(
    ...
    if unmatched_component_keys:
        examples = ", ".join(
            f"{trial_id}/{cohort}"
            for trial_id, cohort in unmatched_component_keys[:20]
        )
        LOGGER.warning(
            "%s has %d trial_id + cohort key(s) absent from the export base; "
...
def _inherit_general_cancer_type_terms_into_explicit_cohorts(
    ...
    general_by_trial = general_rows.set_index("_trial_id_key", drop=False)
    changed_rows = 0

    explicit_mask = ~out[COHORT_COLUMN].eq(GENERAL_COHORT)
    for idx, row in out.loc[explicit_mask].iterrows():
        trial_id = _normalize_anzctr_trial_id(row.get("_trial_id_key", ""))
        if not trial_id or trial_id not in general_by_trial.index:
            continue

        general = general_by_trial.loc[trial_id]
...
def _order_output_columns(out: pd.DataFrame) -> pd.DataFrame:
    base_columns = [
        ...
        if column not in {"_trial_id_key", "_cohort_key", *FINAL_OUTPUT_COLUMNS}
           and column != COHORT_COLUMN
    ]

    if OUTPUT_TRIAL_ID_COLUMN in base_columns:
        insert_at = base_columns.index(OUTPUT_TRIAL_ID_COLUMN) + 1
    else:
        insert_at = 0
...
def build_cohort_resource_table(
    ...
    duplicate_keys = out.loc[
        out.duplicated([output_trial_id_col, COHORT_COLUMN], keep=False),
        [output_trial_id_col, COHORT_COLUMN],
    ]
    if not duplicate_keys.empty:
        examples = [
            f"{getattr(row, output_trial_id_col)}/{getattr(row, COHORT_COLUMN)}"
            for row in duplicate_keys.drop_duplicates().itertuples(index=False)
        ]
        LOGGER.warning(
            "Final cohort resource has duplicate trialId + cohort rows for %d key(s). "
            "Examples: %s",
            len(examples),
            ", ".join(examples[:20]),
        )
    ...
    LOGGER.info(
        ...
        len(out),
        len(out.columns),
        out[output_trial_id_col]
        .map(_normalize_anzctr_trial_id)
        .replace("", pd.NA)
        .dropna()
        .nunique()
        if output_trial_id_col in out.columns
        else 0,
        out[[output_trial_id_col, COHORT_COLUMN]].drop_duplicates().shape[0]
        if {output_trial_id_col, COHORT_COLUMN}.issubset(out.columns)
        else 0,
    )
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build the final ANZCTR cohort-level resource by expanding the base "
            "ANZCTR field extraction table to trial_id + cohort rows, then merging "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Curated ACTRN*.py rules path used to build the cohort base when "
            "--cohort_base_file is not supplied. Defaults to "
            "data/trial_inputs/anzctr/eligibility_curations."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional prebuilt cohort base table with trial_id and cohort columns. "
    ...
    parser.add_argument(
        "--fail_on_error",
        action="store_true",
        help="Fail if any curated ACTRN*.py file cannot be loaded while building the cohort base.",
    )
