...
COHORT_LEVEL_CONFLICTS_STEM = "diagnostics/01b_gene_alteration_conflicts_cohort_level"
...
def collapse_effective_gene_alteration_rows_to_cohort_level(
    ...
    *,
    ...
    """
    Collapse already-expanded effective rows to trial_id + cohort.
...
def collapse_to_cohort_level_gene_alteration(
    *,
    ...
    """
    Collapse mapped GeneAlterationCriterion rows to trial_id + cohort.
...
def _insert_or_replace_cohort_column(
    ...
    *,
    ...
    if "trial_id" in out.columns:
        insert_at = list(out.columns).index("trial_id") + 1
    else:
        insert_at = 1
...
def build_cohort_level_gene_alteration_conflict_report_from_effective_rows(
    ...
    The existing conflict builder is reused within each trial_id + cohort group.
    ...
    _require_columns(
        effective_df,
        ["trial_id", "cohort", "polarity", "gene_alteration_curation"],
    ...
    for (trial_id, cohort), group in grouped.items():
        if group.empty:
            continue
    ...
    # immediately after trial_id when possible.
    columns = list(out.columns)
    if "cohort" in columns and "trial_id" in columns:
        columns.remove("cohort")
        insert_at = columns.index("trial_id") + 1
