...
from aus_trial_universe.core.paths import CURRENT_VERSION, TRIAL_ARMS_ROOT, current_version_dir
from aus_trial_universe.tasks.shared.schema import TABLE_FILES, TRIAL_ARMS_COLUMNS, TrialArm
...
class TrialArmStore:
    ...
    @classmethod
    def load_dir(cls, vdir: Path) -> "TrialArmStore":
        """Load from an EXPLICIT dir holding trial_arms.tsv (used for the expired/ area)."""
        store = cls()
        for row in _read_tsv(Path(vdir) / TABLE_FILES["trial_arms"]):
            a = TrialArm(**{k: row.get(k, "") for k in TRIAL_ARMS_COLUMNS})
            if a.trial_arm_id and a.trialId:
                store.arms.setdefault(a.trialId, []).append(a)
        return store
    ...
    def pop_trial(self, trial_id: str) -> list[TrialArm]:
        """Remove and RETURN a trial's arm rows (used by expiry to move a trial to the expired/ area)."""
        return self.arms.pop(trial_id, [])

    def save_dir(self, vdir: Path) -> Path:
        """Write trial_arms.tsv directly into ``vdir`` (no current_version nesting) — used for the expired/ area."""
        vdir = Path(vdir)
