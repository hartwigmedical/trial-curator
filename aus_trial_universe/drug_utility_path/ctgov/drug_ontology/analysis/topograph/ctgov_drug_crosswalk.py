...
from aus_trial_universe.drug_utility_path.ctgov.drug_ontology.shared.text import (
    clean_text,
    normalize_key as norm_key,
    ordered_join as join_values,
    ordered_unique,
)
from aus_trial_universe.drug_utility_path.ctgov.drug_ontology.shared.tsv import (
    read_tsv as read_tsv_with_header,
    write_tsv as write_rows_to_tsv,
)

CTGOV_DEFAULT_ANNOTATED = Path(
    "data/ctgov/drug_ontology/processed_inputs/analysis/ctgov/version_13022026/ctgov_unique_drugs.atc_annotated.tsv"
)
TOPOGRAPH_DEFAULT_ANNOTATED = Path(
    "data/ctgov/drug_ontology/processed_inputs/analysis/Topograph/version_13052026/topograph_unique_drugs.atc_annotated.tsv"
)
DEFAULT_OUTPUT = Path(
    "data/ctgov/drug_ontology/analysis_outputs/topograph/version_13052026/ctgov_vs_topograph_drugs.tsv"
)
...
def read_tsv(path: Path) -> list[dict[str, str]]:
    ...
    _fieldnames, rows = read_tsv_with_header(path)
    return rows
