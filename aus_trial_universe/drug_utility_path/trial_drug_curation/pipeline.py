...
from aus_trial_universe.drug_utility_path.trial_drug_curation.prompts.prompt_builder import build_prompt
...
from aus_trial_universe.drug_utility_path.trial_drug_curation.utils.local_records import (
    load_trial_records,
)
from aus_trial_universe.drug_utility_path.trial_drug_curation.utils.outputs import (
    count_tsv_data_rows,
    dated_output_path,
    merge_tsv_tables,
...
from aus_trial_universe.drug_utility_path.trial_drug_curation.utils.resources import (
...
OncotreeEnricherFactory = Callable[[str], OncotreeEnricher]
...
class CurationRunConfig:
    ...
    # Fill the Oncotree column with the agentic workflow instead of the LLM's
    # inline guess. Disable for cheap/offline curation runs.
    enrich_oncotree: bool = True
...
def run_trial_drug_curation(
    ...
    *,
    ...
    oncotree_enricher_factory: OncotreeEnricherFactory = OncotreeEnricher.from_yaml,
) -> CurationRunResult:
    ...
    oncotree_enricher = None
    if config.enrich_oncotree:
        _log(progress, "Loading OncoTree agentic workflow for OncoTree column mapping...")
        oncotree_enricher = oncotree_enricher_factory(config.oncotree_yaml)
    ...
    if oncotree_enricher is not None:
        _log(progress, "Mapping OncoTree codes from cancerTypes via the agentic workflow...")
        tsv = oncotree_enricher.enrich_tsv(tsv)
