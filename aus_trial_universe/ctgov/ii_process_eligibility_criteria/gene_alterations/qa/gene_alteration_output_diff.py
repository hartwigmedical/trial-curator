...
DEFAULT_PROCESSED_DIR = Path("data/ctgov/eligibility/processed/gene_alteration")
DEFAULT_BASELINE_SUBDIR = "baseline"
DEFAULT_DATE_FORMAT = "%d%m%Y"

OUTPUT_FILES: Sequence[str] = (
    "01_gene_alteration_mapping_resource.tsv",
    "02_gene_alteration_manual_filter.tsv",
...
KEY_COLUMNS_BY_FILE: Dict[str, Sequence[str]] = {
    ...
    # Current human-readable 05 uses these. If an older 05 schema is supplied,
    # the fallback below will pick usable columns.
    "05_gene_alteration_conflicts.tsv": (
        "nct_id",
        "positive_term",
        "negative_term",
...
def _choose_key_columns(filename: str, before: pd.DataFrame, after: pd.DataFrame) -> List[str]:
    ...
    # Fallbacks for possible older 05 schema.
    fallback_sets: List[Sequence[str]] = [
        ("nct_id", "conflict_term"),
        ("conflict_group_id", "nct_id"),
        ("nct_id",),
    ]
...
def _add_row_key(df: pd.DataFrame, key_columns: Sequence[str]) -> pd.DataFrame:
    ...
    # Make duplicate keys deterministic rather than silently collapsing them.
        ...
        rows.append(
            {
                ...
                "column": "*",
                "baseline_value": "",
                "new_value": _row_to_json(after_keyed.loc[key].drop(labels=["_diff_key_base", "_diff_key"])),
            }
        )
        ...
        rows.append(
            {
                ...
                "row_key": key,
                "column": "*",
                "baseline_value": _row_to_json(before_keyed.loc[key].drop(labels=["_diff_key_base", "_diff_key"])),
                "new_value": "",
            }
        )
...
def create_gene_alteration_snapshot_and_diffs(
    *,
    processed_dir: Path,
    baseline_dir: Path,
    snapshot_dir: Path,
    ...
    for filename in files:
        ...
        if current_file.exists():
            ...
            LOGGER.warning("Current output file does not exist and will not be copied: %s", current_file)
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Snapshot current gene-alteration processed outputs into a dated folder "
    ...
    parser.add_argument(
        ...
        help="Gene-alteration processed output directory.",
    )
    parser.add_argument(
        "--baseline_dir",
        type=Path,
        default=None,
        help=(
            "Baseline directory to compare against. Defaults to "
            "processed_dir/baseline."
        ),
    )
    parser.add_argument(
        "--snapshot_dir",
        ...
        default=None,
        help=(
            "Output snapshot directory. Defaults to processed_dir/<ddmmyyyy>."
        ),
    )
    parser.add_argument(
        "--snapshot_date",
    ...
    summary_df = create_gene_alteration_snapshot_and_diffs(
