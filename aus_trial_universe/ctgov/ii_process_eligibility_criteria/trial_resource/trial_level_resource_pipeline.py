...
DEFAULT_TRIALS_FILE = Path("trials/ctgov_field_extractions.csv")
DEFAULT_CANCER_TYPE_FILE = Path("processed/cancer_type/04_trial_level_cancer_type.tsv")
DEFAULT_GENE_ALTERATION_FILE = Path("processed/gene_alteration/04_trial_level_gene_alteration.tsv")
...
NCT_ID_RE = re.compile(r"NCT\d{8}", flags=re.IGNORECASE)

NCT_ID_COLUMN_CANDIDATES: Sequence[str] = (
    "nct_id",
    "nctId",
    "NCTId",
    "nctid",
...
def _normalize_nct_id(value: object) -> str:
    if value is None:
        return ""
...
def _find_nct_id_column(df: pd.DataFrame, explicit_column: Optional[str] = None) -> str:
    columns = [str(column).strip() for column in df.columns]

    if explicit_column is not None:
        if explicit_column not in columns:
            raise ValueError(
                f"Explicit NCT ID column {explicit_column!r} not found. "
    ...
    for candidate in NCT_ID_COLUMN_CANDIDATES:
    ...
    raise ValueError(
        "Could not infer NCT ID column. "
        f"Tried candidates: {list(NCT_ID_COLUMN_CANDIDATES)}. "
        f"Available columns: {columns}"
    )
...
def _load_base_trials(
    ...
    *,
    ...
    key_column = _find_nct_id_column(df, nct_id_column)

    out = df.copy()
    out["_nct_id_key"] = out[key_column].map(_normalize_nct_id)

    blank_keys = int((out["_nct_id_key"] == "").sum())
    if blank_keys:
    ...
    if not duplicate_keys.empty:
        LOGGER.warning(
            "Base trials file has %d duplicate NCT ID row(s), affecting %d trial(s). "
    ...
    LOGGER.info(
        "Loaded base trials: %s rows=%d unique_trials=%d nct_id_column=%s",
        trials_file,
        len(out),
        out["_nct_id_key"].replace("", pd.NA).dropna().nunique(),
        key_column,
    )
...
def _load_component_table(
    ...
    *,
    ...
    out = out[out["_nct_id_key"] != ""].copy()

    duplicated = out.loc[out["_nct_id_key"].duplicated(keep=False), "_nct_id_key"]
    if not duplicated.empty:
        duplicate_examples = ", ".join(sorted(duplicated.unique())[:20])
        raise ValueError(
            f"{label} has duplicate nct_id rows for {duplicated.nunique()} trial(s): "
            f"{duplicate_examples}"
        )

    out = out.loc[:, ["_nct_id_key", *required_value_columns]]
...
def build_trial_resource_table(
    *,
    ...
    out = out.drop(columns=["_nct_id_key"], errors="ignore")
...
def discover_pipeline_inputs(
    *,
    ...
    resolved_trials_file = _resolve_path(
        trials_file if trials_file is not None else eligibility_dir / DEFAULT_TRIALS_FILE,
        repo_root,
    )
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build the final CTGov trial resource by merging the reviewed "
            "trial-level cancer type, gene alteration, and molecular signature outputs "
            "onto the base CTGov field extraction table. The pipeline writes one "
            "dated TSV export."
        )
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "eligibility_data_dir/trials/ctgov_field_extractions.csv."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "eligibility_data_dir/exports/trial_resource_<ddmmyyyy>.tsv."
        ),
    )
