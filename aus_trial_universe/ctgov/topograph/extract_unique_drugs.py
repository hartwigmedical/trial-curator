...
def find_topograph_input_file(input_dir: Path) -> Path:
    """Find a Topograph oncotree TSV in a directory."""
    if not input_dir.exists():
        raise FileNotFoundError(f"Input directory does not exist: {input_dir}")
    if not input_dir.is_dir():
        raise NotADirectoryError(f"--input-dir must be a directory: {input_dir}")

    candidates = sorted(input_dir.glob("TOPOGRAPH-oncotree-*.tsv"))
    if not candidates:
        raise FileNotFoundError(
            f"No Topograph oncotree TSV found in {input_dir}. "
            "Expected a file like TOPOGRAPH-oncotree-21052026.tsv"
        )
    if len(candidates) > 1:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    input_group.add_argument(
        ...
        dest="input_file",
        type=Path,
        help="Topograph TSV path, e.g. TOPOGRAPH-oncotree-21052026.tsv.",
    )
    input_group.add_argument(
        "--input-dir",
        "--input_dir",
        dest="input_dir",
        type=Path,
        help="Directory containing exactly one TOPOGRAPH-oncotree-*.tsv file.",
    )
    parser.add_argument(
        "--drug-column",
        "--drug_column",
