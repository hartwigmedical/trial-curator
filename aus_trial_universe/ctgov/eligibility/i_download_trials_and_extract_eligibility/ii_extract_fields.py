...
def extract_fields_to_outputs(ctgov_filepath: Path, output_dir: Path) -> tuple[Path, Path]:
    ...
    xlsx_path = output_dir / "ctgov_field_extractions.xlsx"
...
def main():
    ...
    parser.add_argument("--output_dir", help="Directory to store the output xlsx workbook and selected-trials CSV", required=True)
    ...
    extract_fields_to_outputs(Path(args.ctgov_filepath), Path(args.output_dir))
